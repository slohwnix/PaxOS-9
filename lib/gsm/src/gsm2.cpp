...
namespace Gsm
{
    ...
    namespace Time
    {
        ...
        int getYear()
        {
            ...
            #else
                if (timeValid) return year;
                std::time_t t = std::time(nullptr);
                std::tm* now = std::localtime(&t);
                return now ? (now->tm_year + 1900) : -1; // tm_year depuis 1900
            #endif
        }
        ...
        int getMonth()
        {
            ...
            #else
                if (timeValid) return month;
                std::time_t t = std::time(nullptr);
                std::tm* now = std::localtime(&t);
                return now ? now->tm_mon + 1 : -1; // tm_mon de 0 à 11, donc +1
            #endif
        }
